...
function washResult(result) {
  return _.pick(result, ['id', 'content', 'status', 'remark', 'tradeId', 'userId', 'user', 'trade', 'createdAt', 'updatedAt']);
}
...
exports.put = function(req, res, next) {
  ...
  let check = Validator
                .isObject()
                .withOptional('status', Validator.isNumber())
                .withOptional('remark', Validator.isString());
  // 验证body
  body = _.pick(body, ['status', 'remark']);
