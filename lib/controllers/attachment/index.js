...
function washResult(result) {
  return _.pick(result, ['id', 'name', 'ext', 'key', 'cost', 'courseId', 'authorId', 'createdAt', 'updatedAt']);
}
...
exports.get = function(req, res, next) {
  ...
  Attachment.findAndCountAll(queryOptions).then((result) => {
...
exports.getById = function(req, res, next) {
  ...
  let resData = {};
  Attachment.findById(id).then((result) => {
    if(result) {
      resData.success = true;
...
exports.post = function(req, res, next) {
  ...
  // 验证body
  body = _.pick(body, ['name', 'ext', 'key', 'cost', 'courseId']);
  Validator.run(check, body, function(errorCount, errors) {
    ...
    // 查询key的唯一性
    Attachment.count({ where: {key: body.key} }).then((keyExisted) => {
      if(keyExisted) {
        resData.success = false;
        resData.error = {
      ...
      Attachment.create(body).then((result) => {
        resData.success = true;
        resData.result = washResult(result);
        return res.send(resData);
      }).catch(util.sendError);
...
exports.put = function(req, res, next) {
  ...
  Validator.run(check, body, function(errorCount, errors) {
    ...
    // 检查id是否存在
    Attachment.findById(id).then((workingAttachment) => {
      if(!workingAttachment) {
        resData.success = false;
        resData.error = {
          message: `Attachment "${id}" has not existed.`
        };
        return res.send(resData);
      }
      workingAttachment.update(body).then(function(result) {
        resData.success = true;
        resData.result = washResult(result)
        return res.send(resData);
      }).catch(util.sendError);
...
exports.delete = function(req, res, next) {
  ...
  Attachment.findById(id).then((workingAttachment) => {
    ...
    workingAttachment.destroy().then((result) => {
      resData.success = true;
      resData.result = washResult(result)
      return res.send(resData);
    }).catch(util.sendError);
