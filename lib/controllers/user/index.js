...
function washResult(result) {
  return _.pick(result, ['id', 'phone', 'nickname', 'gender', 'icon', 'email', 'qq', 'wechat', 'introduction', 'enable', 'createdAt', 'updatedAt']);
}
...
exports.post = function(req, res, next) {
  ...
  resData.success = false;
  resData.error = {
    message: 'Cannot create any user'
  };
...
exports.put = function(req, res, next) {
  ...
  Validator.run(check, body, function(errorCount, errors) {
    ...
    // 检查id是否存在
    User.findById(id).then((workingUser) => {
      if(!workingUser) {
        resData.success = false;
        resData.error = {
