...
function washResult(result) {
  return _.pick(result, ['id', 'name', 'createdAt', 'updatedAt', 'schoolId', 'school']);
}
...
exports.get = function(req, res, next) {
  ...
  queryOptions.include = [ School ];
  
  Speciality.findAndCountAll(queryOptions).then((result) => {
...
exports.post = function(req, res, next) {
  ...
  let check = Validator
                ...
                .withRequired('schoolId', Validator.isNumber());
  // 验证body
  body = _.pick(body, ['name', 'schoolId']);
  Validator.run(check, body, function(errorCount, errors) {
    ...
    Speciality.create(body).then((result) => {
      resData.success = true;
      resData.result = washResult(result);
      return res.send(resData);
    }).catch(util.sendError);
...
exports.put = function(req, res, next) {
  ...
  let check = Validator
                ...
                .withOptional('schoolId', Validator.isNumber());
  // 验证body
  body = _.pick(body, ['name', 'schoolId']);
  Validator.run(check, body, function(errorCount, errors) {
    if(errorCount) {
    ...
    // 检查id是否存在
    Speciality.findById(id).then((workingSpeciality) => {
      if(!workingSpeciality) {
        resData.success = false;
        resData.error = {
          message: `Speciality "${id}" has not existed.`
        };
        return res.send(resData);
      }
      workingSpeciality.update(body).then(function(result) {
        resData.success = true;
        resData.result = washResult(result)
        return res.send(resData);
      }).catch(util.sendError);
...
exports.delete = function(req, res, next) {
  ...
  Speciality.findById(id).then((workingSpeciality) => {
    ...
    workingSpeciality.destroy().then((result) => {
      resData.success = true;
      resData.result = washResult(result)
      return res.send(resData);
    }).catch(util.sendError);
